...
public interface IProductInfoRepository extends JpaRepository<ProductInfo,Integer> {
    ...
    @Query("SELECT i FROM ProductInfo i JOIN i.categorySet c WHERE c.id IN (:categoryIds) AND i.business.user.state = :state_business")
    ...
    Page<ProductInfo> findBySaleDiscount(Pageable pageable, Integer state, Double discount, Integer state_business);
